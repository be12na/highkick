function getIuranBulananByAnggota_(spreadsheetId, nomorAnggota) {
  const target = normalize_(nomorAnggota);
  return getAllObjects_(spreadsheetId, SHEET_NAMES.iuranBulanan)
...
function saveIuranBulanan_(spreadsheetId, payload) {
  ...
  const nominalTagihan = resolveNominalByType_(spreadsheetId, 'bulanan', periode, payload.nominal_tagihan);
  ...
  const finalPayload = Object.assign({}, payload, {
    transaksi_id: normalize_(payload.transaksi_id) || generateId_('ibl'),
  ...
  appendObject_(spreadsheetId, SHEET_NAMES.iuranBulanan, finalPayload);
  return finalPayload;
}
